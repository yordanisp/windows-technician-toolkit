...
REM v1.4 - Nine organized main categories with health check placed under System.
REM v1.5 - Network discovery, remote monitoring, DNS diagnostics, and bandwidth tools.
REM v1.6 - Driver inventory, device problems, firmware information, and driver backup.
REM ============================================================
...
cd /d "%~dp0"
set "TOOLKIT_VERSION=1.6"
set "TOOLKIT_RELEASE=Driver and firmware diagnostics"
...
echo !GREEN!8^) Security and administration!RESET!
echo !GREEN!9^) Windows tools and power!RESET!
...
if "%option%"=="8" goto security_menu
...
echo !GREEN!5^) System health check!RESET!
echo !GREEN!6^) Drivers and firmware!RESET!
...
if "%choice%"=="5" call :health_check
if "%choice%"=="6" goto drivers_menu
...
:drivers_menu
cls
echo !YELLOW!=== Drivers and firmware ===!RESET!
echo !GREEN!1^) Installed driver inventory!RESET!
echo !GREEN!2^) Devices with problems or missing drivers!RESET!
echo !GREEN!3^) Driver signature check!RESET!
echo !GREEN!4^) Printer drivers!RESET!
echo !GREEN!5^) Monitor information!RESET!
echo !GREEN!6^) Firmware information!RESET!
echo !GREEN!7^) Scan Windows Update for drivers!RESET!
echo !GREEN!8^) Backup installed drivers!RESET!
echo !GREEN!9^) Export driver report!RESET!
echo !GREEN!0^) System information menu!RESET!
set "choice="
set /p "choice=Choose an option: "
if "%choice%"=="1" call :driver_inventory
if "%choice%"=="2" call :device_problems
if "%choice%"=="3" call :driver_signatures
if "%choice%"=="4" call :printer_drivers
if "%choice%"=="5" call :monitor_information
if "%choice%"=="6" call :firmware_information
if "%choice%"=="7" call :driver_update_scan
if "%choice%"=="8" call :backup_drivers
if "%choice%"=="9" call :export_driver_report
...
echo !GREEN!9^) Advanced network diagnostics!RESET!
echo !GREEN!0^) Main menu!RESET!
set "choice="
set /p "choice=Choose an option: "
...
if "%choice%"=="9" goto network_tools_menu
if "%choice%"=="0" goto menu
if not "%choice%"=="0" pause
goto network_menu
...
:network_tools_menu
cls
echo !YELLOW!=== Advanced network diagnostics ===!RESET!
echo !GREEN!1^) Ping sweep a subnet!RESET!
echo !GREEN!2^) Test common TCP ports!RESET!
echo !GREEN!3^) DNS and domain diagnostics!RESET!
echo !GREEN!4^) Network adapter bandwidth!RESET!
echo !GREEN!5^) Monitor a remote computer!RESET!
echo !GREEN!6^) Trace route to a host!RESET!
echo !GREEN!7^) Remote bandwidth monitor!RESET!
echo !GREEN!0^) Network menu!RESET!
set "choice="
set /p "choice=Choose an option: "
if "%choice%"=="1" call :ping_sweep
if "%choice%"=="2" call :port_test
if "%choice%"=="3" call :dns_diagnostics
if "%choice%"=="4" call :bandwidth_info
if "%choice%"=="5" call :remote_monitor
if "%choice%"=="6" call :trace_host
if "%choice%"=="7" call :remote_bandwidth
...
echo !GREEN!7^) Power options!RESET!
echo !GREEN!0^) Main menu!RESET!
set "choice="
set /p "choice=Choose an option: "
...
:ping_sweep
echo !YELLOW!Use a local subnet in CIDR notation, for example 192.168.1.0/24.!RESET!
set "subnet="
set /p "subnet=Subnet: "
powershell -NoProfile -Command "$cidr='%subnet%'; if($cidr -notmatch '^((25[0-5]|2[0-4]\d|1?\d?\d)\.){3}(25[0-5]|2[0-4]\d|1?\d?\d)/(3[0-2]|[12]?\d)$'){Write-Error 'Invalid CIDR notation.'; exit 1}; $base,$prefix=$cidr -split '/'; $ip=[Net.IPAddress]::Parse($base); $bytes=$ip.GetAddressBytes(); [Array]::Reverse($bytes); $network=[BitConverter]::ToUInt32($bytes,0); $hostBits=32-[int]$prefix; $count=[math]::Pow(2,$hostBits)-2; if($count -gt 1022){$count=1022}; 1..$count | ForEach-Object { $number=$network+$_; $address=[BitConverter]::GetBytes([uint32]$number); [Array]::Reverse($address); $target=([Net.IPAddress]::new($address)).ToString(); if(Test-Connection -ComputerName $target -Count 1 -Quiet -TimeoutSeconds 1){[PSCustomObject]@{Address=$target; Status='Online'}} } | Format-Table -AutoSize"
exit /b

:port_test
set "target="
set /p "target=Host or IP address: "
echo Testing common ports on %target%...
powershell -NoProfile -Command "$target='%target%'; $ports=@{DNS=53;HTTP=80;HTTPS=443;SMB=445;RDP=3389;WinRM=5985;SCCM=10123}; $ports.GetEnumerator() | ForEach-Object {$open=Test-NetConnection -ComputerName $target -Port $_.Value -InformationLevel Quiet -WarningAction SilentlyContinue; [PSCustomObject]@{Port=$_.Value; Service=$_.Key; Open=$open}} | Format-Table -AutoSize"
exit /b

:dns_diagnostics
set "domain="
set /p "domain=Host or domain name: "
powershell -NoProfile -Command "Resolve-DnsName '%domain%' -ErrorAction SilentlyContinue | Select Name,Type,IPAddress,NameHost,TTL | Format-Table -AutoSize"
nslookup %domain%
exit /b

:bandwidth_info
powershell -NoProfile -Command "Get-NetAdapterStatistics | Select Name,ReceivedBytes,SentBytes,@{N='ReceivedGB';E={[math]::Round($_.ReceivedBytes/1GB,2)}},@{N='SentGB';E={[math]::Round($_.SentBytes/1GB,2)}} | Format-Table -AutoSize"
echo.
echo Live counters are cumulative since the adapter was started.
exit /b

:remote_monitor
set "remote="
set /p "remote=Remote computer name: "
powershell -NoProfile -Command "$computer='%remote%'; if(-not (Test-Connection -ComputerName $computer -Count 1 -Quiet)){Write-Error 'Remote computer is unreachable.'; exit 1}; $os=Get-CimInstance Win32_OperatingSystem -ComputerName $computer -ErrorAction SilentlyContinue; $system=Get-CimInstance Win32_ComputerSystem -ComputerName $computer -ErrorAction SilentlyContinue; Write-Output ('Monitoring {0}. Press Ctrl+C to stop.' -f $computer); if(-not $os -or -not $system){Write-Warning 'WMI data is unavailable; continuing with ping monitoring only.'}; 1..10 | ForEach-Object {$reply=Test-Connection -ComputerName $computer -Count 1 -ErrorAction SilentlyContinue; [PSCustomObject]@{Time=(Get-Date -Format 'HH:mm:ss'); Computer=$computer; Status=if($reply){'Online'}else{'Offline'}; LatencyMs=if($reply){$reply.Latency}else{'-'}; OS=if($os){$os.Caption}else{'Unavailable'}} | Format-Table -AutoSize; Start-Sleep -Seconds 3}"
exit /b

:remote_bandwidth
set "remote="
set /p "remote=Remote computer name: "
powershell -NoProfile -Command "$computer='%remote%'; $localNames=@('localhost','.','127.0.0.1',$env:COMPUTERNAME); if(-not (Test-Connection -ComputerName $computer -Count 1 -Quiet)){Write-Error 'Computer is unreachable.'; exit 1}; try{if($localNames -contains $computer){$counter=Get-Counter '\Network Interface(*)\Bytes Total/sec' -ErrorAction Stop}else{$counter=Get-Counter -ComputerName $computer -Counter '\Network Interface(*)\Bytes Total/sec' -ErrorAction Stop}; $counter.CounterSamples | Where-Object InstanceName -notmatch 'Loopback|isatap|Teredo' | Select-Object @{N='Adapter';E={$_.InstanceName}},@{N='Mbps';E={[math]::Round(($_.CookedValue*8)/1MB,2)}} | Sort-Object Mbps -Descending | Format-Table -AutoSize}catch{Write-Error ('Performance counters unavailable for ' + $computer + '. Check Remote Registry, firewall, and permissions. Details: ' + $_.Exception.Message); exit 1}"
exit /b

:trace_host
set "traceTarget="
set /p "traceTarget=Host or IP address: "
tracert -d -h 20 %traceTarget%
exit /b
...
:driver_inventory
powershell -NoProfile -Command "Get-CimInstance Win32_PnPSignedDriver -ErrorAction SilentlyContinue | Where-Object DeviceName | Select DeviceName,Manufacturer,DriverVersion,DriverDate,IsSigned,InfName | Sort DeviceName | Format-Table -AutoSize"
exit /b

:device_problems
powershell -NoProfile -Command "$devices=Get-CimInstance Win32_PnPEntity -ErrorAction SilentlyContinue | Where-Object {$_.ConfigManagerErrorCode -and $_.ConfigManagerErrorCode -ne 0}; if($devices){$devices | Select Name,PNPClass,Manufacturer,ConfigManagerErrorCode,Status | Format-Table -AutoSize}else{Write-Output 'No devices with reported driver problems were found.'}"
exit /b

:driver_signatures
powershell -NoProfile -Command "$unsigned=Get-CimInstance Win32_PnPSignedDriver -ErrorAction SilentlyContinue | Where-Object {$_.DeviceName -and $_.IsSigned -ne $true}; if($unsigned){$unsigned | Select DeviceName,Manufacturer,DriverVersion,DriverDate,IsSigned,InfName | Format-Table -AutoSize}else{Write-Output 'All detected Plug and Play drivers are signed.'}"
exit /b

:printer_drivers
powershell -NoProfile -Command "Get-CimInstance Win32_Printer -ErrorAction SilentlyContinue | Select Name,DriverName,PortName,PrinterStatus,WorkOffline | Format-Table -AutoSize"
exit /b

:monitor_information
powershell -NoProfile -Command "$monitors=Get-CimInstance -Namespace root\wmi -ClassName WmiMonitorID -ErrorAction SilentlyContinue; if($monitors){$monitors | ForEach-Object {[PSCustomObject]@{Manufacturer=((-join ($_.ManufacturerName | Where-Object {$_ -ne 0})) ); Model=((-join ($_.UserFriendlyName | Where-Object {$_ -ne 0})) ); Serial=((-join ($_.SerialNumberID | Where-Object {$_ -ne 0})) )}} | Format-Table -AutoSize}else{Get-CimInstance Win32_DesktopMonitor | Select Name,PNPDeviceID,MonitorType | Format-Table -AutoSize}"
exit /b

:firmware_information
powershell -NoProfile -Command "$bios=Get-CimInstance Win32_BIOS; $disks=Get-PhysicalDisk -ErrorAction SilentlyContinue; [PSCustomObject]@{BIOSVersion=$bios.SMBIOSBIOSVersion; BIOSReleaseDate=$bios.ReleaseDate; BIOSSerial=$bios.SerialNumber}; $disks | Select FriendlyName,SerialNumber,FirmwareVersion,HealthStatus,OperationalStatus | Format-Table -AutoSize"
exit /b

:driver_update_scan
echo !YELLOW!Windows Update will be opened to check for available driver updates.!RESET!
start "" ms-settings:windowsupdate
exit /b

:backup_drivers
set "driverBackup=%REPORT_DIR%\DriverBackup_%COMPUTERNAME%"
if not exist "%driverBackup%" mkdir "%driverBackup%"
echo !BLUE!Exporting installed drivers to %driverBackup%...!RESET!
call :admin_required pnputil /export-driver * "%driverBackup%"
echo !GREEN!Driver backup completed.!RESET!
exit /b

:export_driver_report
set "driverReport=%REPORT_DIR%\Drivers_%COMPUTERNAME%.csv"
powershell -NoProfile -Command "Get-CimInstance Win32_PnPSignedDriver -ErrorAction SilentlyContinue | Where-Object DeviceName | Select DeviceName,Manufacturer,DriverVersion,DriverDate,IsSigned,InfName | Sort DeviceName | Export-Csv -Path '%driverReport%' -NoTypeInformation -Encoding UTF8"
echo !GREEN!Driver report saved to: %driverReport%!RESET!
exit /b
